...
import com.fishfishfish.fishaccounting.widget.SwipeMenuView;
import com.fishfishfish.fishaccounting.widget.stickyheader.StickyHeaderGridAdapter;

import java.util.List;

import static com.fishfishfish.fishaccounting.utils.DateUtils.FORMAT_HMS_CN;
import static com.fishfishfish.fishaccounting.utils.DateUtils.FORMAT_YMD_CN;
...
public class MonthDetailAdapter extends StickyHeaderGridAdapter {
    ...
    public void onBindItemViewHolder(ItemViewHolder viewHolder, final int section, final int position) {
        ...
        FishBill fishBill = mDatas.get(section).getList().get(position);
        holder.item_title.setText(fishBill.getSortName());
        holder.item_img.setImageDrawable(ImageUtils.getDrawable(fishBill.getSortImg()));
        if (fishBill.isIncome()) {
            holder.item_money.setText("+" + fishBill.getCost());
        } else {
            holder.item_money.setText("-" + fishBill.getCost());
        }
...
//                确认删除
        ...
        holder.item_layout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                FishBill fishBill = mDatas.get(section).getList().get(offset);

                iv_bill.setImageDrawable(ImageUtils.getDrawable(fishBill.getSortImg()));
                String content = fishBill.getContent();
                if (content != null) tv_content.setText("备注信息：" + content);
                tv_title.setText("因" + fishBill.getSortName() + "消费" + Math.abs(fishBill.getCost()) + "元");
                if (fishBill.isIncome())
                    tv_title.setText("因" + fishBill.getSortName() + "收入" + fishBill.getCost() + "元");
                tv_date.setText(DateUtils.long2Str(fishBill.getCrdate(), FORMAT_YMD_CN)
                        + "\n\n" + DateUtils.long2Str(fishBill.getCrdate(), FORMAT_HMS_CN));
    ...
    public interface OnStickyHeaderClickListener {
        void OnDeleteClick(FishBill item, int section, int offset);

        void OnEditClick(FishBill item, int section, int offset);
    }
